...
export default function SupplierListPage() {
  ...
  const filteredAndSortedSuppliers = useMemo(() => {
    ...
    if (activeTab === 'nearby' && userLocation) {
      ...
      if (sortBy === 'rating') {
        ...
        result.sort((a, b) => (b.supplierReviewCount || 0) - (a.supplierReviewCount || 0));
      }
    }

    return result;
  }, [suppliers, sortBy, userLocation, activeTab]);
  ...
  const paginatedMainSuppliers = useMemo(() => {
    const startIndex = (currentPageMain - 1) * ITEMS_PER_PAGE_MAIN;
    return filteredAndSortedSuppliers.slice(startIndex, startIndex + ITEMS_PER_PAGE_MAIN);
  }, [filteredAndSortedSuppliers, currentPageMain]);

  const paginatedSidebarSuppliers = useMemo(() => {
    const startIndex = (currentPageSidebar - 1) * ITEMS_PER_PAGE_SIDEBAR;
    return nearbyFilteredSuppliers.slice(startIndex, startIndex + ITEMS_PER_PAGE_SIDEBAR);
  }, [nearbyFilteredSuppliers, currentPageSidebar]);

  const totalPagesMain = Math.ceil(filteredAndSortedSuppliers.length / ITEMS_PER_PAGE_MAIN);
  const totalPagesSidebar = Math.ceil(nearbyFilteredSuppliers.length / ITEMS_PER_PAGE_SIDEBAR);

  useEffect(() => {
    setCurrentPageMain(1);
  }, [activeTab, sortBy, searchTerm, selectedDistrict]);

  useEffect(() => {
    setCurrentPageSidebar(1);
  }, [sidebarSearchTerm]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50" data-theme="light">
      ...
      <main className="flex-grow w-full pb-20">
        <section className="relative w-full bg-slate-900 overflow-hidden mb-12 pt-48 pb-24 lg:pt-56 lg:pb-32" data-theme="dark">
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 -mt-20 relative z-20">
          <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-[24px] p-6 shadow-xl border border-white/60 sticky top-24">
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-3 min-h-[200px]">
                    {paginatedSidebarSuppliers.length > 0 ? (
                      paginatedSidebarSuppliers.map(shop => (
                        <div
                          key={shop._id}
                          onClick={() => {
                            if (viewMode !== 'map') {
                      ...
                      <div className="text-center py-6 text-slate-400 text-[10px] font-medium italic">Không có dữ liệu</div>
                    )}
                  </div>
                ...
                <div className="mt-8 p-6 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl text-white relative overflow-hidden" data-theme="dark">
            ...
            <div className="flex-1">
              <div className="bg-white/50 backdrop-blur-md rounded-2xl p-4 mb-8 border border-white/60 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex p-1 bg-slate-100 rounded-xl w-fit">
                    <button
                      onClick={() => setActiveTab('all')}
                      className={`px-6 py-2.5 text-xs font-bold rounded-lg transition-all flex items-center gap-2 ${activeTab === 'all' ? "bg-white text-indigo-600 shadow-md" : "text-slate-500 hover:text-slate-700"}`}
                    >
                      <Store size={14} />
                      Tất cả cửa hàng
                    </button>
                    <button
                      onClick={() => {
                        if (!userLocation) {
                          handleLocateMe().then(() => setActiveTab('nearby'));
                        } else {
                          setActiveTab('nearby');
                        }
                      }}
                      className={`px-6 py-2.5 text-xs font-bold rounded-lg transition-all flex items-center gap-2 ${activeTab === 'nearby' ? "bg-white text-indigo-600 shadow-md" : "text-slate-500 hover:text-slate-700"}`}
                    >
                      <Navigation size={14} />
                      Gần tôi nhất
                    </button>
                  ...
                  <div className="flex items-center gap-4 ml-auto">
                    <div className="flex items-center gap-3">
                      <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Sắp xếp</span>
                      <div className="relative">
                        <select
                          value={sortBy}
                          onChange={(e) => setSortBy(e.target.value)}
                          className="appearance-none bg-white border border-slate-200 text-slate-700 text-xs font-bold py-2 pl-4 pr-10 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer shadow-sm hover:border-indigo-300 transition-colors"
                        >
                          <option value="featured">Nổi bật nhất</option>
                          <option value="rating">Đánh giá sao</option>
                          <option value="newest">Đăng ký mới nhất</option>
                          <option value="popular">Nhiều lượt thuê</option>
                        </select>
                    ...
                    <div className="flex p-1 bg-slate-100 rounded-xl">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all ${viewMode === 'map' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                        title="Dạng bản đồ"
                      >
                        <Navigation size={18} />
                      </button>
              ...
              {viewMode === 'grid' ? (
                <div className="pb-20">
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    ) : paginatedMainSuppliers.length > 0 ? (
                      paginatedMainSuppliers.map((shop) => (
                        <motion.div
                          key={shop._id}
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                          whileHover={{ y: -8 }}
                          onClick={() => navigate(`/supplier/${shop.userId?._id || shop.userId}`)}
                          className="bg-white rounded-[32px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer border border-slate-100 h-full flex flex-col group relative"
                        >
                ...
                <div className="relative h-[800px] w-full rounded-[3.5rem] overflow-hidden shadow-2xl border-[12px] border-white bg-slate-100">
                  <Map
                    ...
                    reuseMaps
                  >
                    {filteredAndSortedSuppliers.map((shop) => (
                      <Marker
                        key={shop._id}
