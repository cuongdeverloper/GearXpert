...
const CartPage = () => {
  ...
  const processCartData = (rawCart) => {
    ...
    const supplierGroups = rawCart.items.reduce((acc, item) => {
      ...
      const supplierName = item.deviceId?.supplierId?.fullName || `Cửa hàng #${supplierId?.slice(-6) || 'unknown'}`;
    ...
    return Object.values(supplierGroups).map(supplierGroup => {
      supplierGroup.devices = Object.values(supplierGroup.devices).map(deviceGroup => {
  ...
  const calculateTotals = () => {
    ...
    groupedBySupplier.forEach(supplier => {
      Object.values(supplier.devices).forEach(deviceGroup => {
        ...
        const rent = deviceGroup.bookings.reduce((sum, b) => 
          sum + (b.rentPrice * b.totalDays * b.quantity), 0
        );
        totalRent += rent;

        // Tiền cọc: max quantity trong tất cả booking của thiết bị này
        const maxQty = Math.max(...deviceGroup.bookings.map(b => b.quantity), 0);
          ...
          <div className="lg:col-span-8 space-y-6">
            {groupedBySupplier.length > 0 ? (
              groupedBySupplier.map((supplierGroup) => (
                <div key={supplierGroup.supplierId} className="bg-white rounded-[40px] p-8 border border-slate-100 shadow-xl hover:shadow-2xl transition-all duration-500 relative group overflow-hidden">
                  ...
                  <div className="relative flex items-center gap-4 mb-6 pb-4 border-b border-slate-100">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Nhà cung cấp thiết bị</p>
                    </div>
                  </div>

                  {/* DEVICES IN SUPPLIER */}
                  {Object.values(supplierGroup.devices).map((deviceGroup) => (
                    <div key={deviceGroup.device._id} className="mb-8 last:mb-0">
                      <div className="flex flex-col md:flex-row gap-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                        ...
                        <div className="w-full md:w-40 h-40 bg-white rounded-2xl overflow-hidden border border-slate-200 flex-shrink-0">
                          <img
                            src={deviceGroup.device?.images?.[0] || 'https://via.placeholder.com/150'}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-4">
                            {deviceGroup.bookings.map((booking) => (
                              <div key={booking.id} className="flex flex-wrap justify-between items-center gap-4 bg-white p-4 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-slate-500">
                                      {booking.totalDays} ngày • SL: {booking.quantity}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-6">
                                  ...
                                  <button
                                    onClick={() => handleRemove(booking.id, deviceGroup.device?.name)}
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100 text-right">
                    <p className="text-sm font-medium text-slate-600">
                      ...
                      <span className="font-bold text-indigo-600 ml-2">
                        {Object.values(supplierGroup.devices).reduce((sum, d) => {
                          return sum + d.bookings.reduce((s, b) => s + (b.rentPrice * b.totalDays * b.quantity), 0);
                        }, 0).toLocaleString()}đ
                      </span>
                    </p>
                  </div>
