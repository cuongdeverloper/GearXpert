    ...
    <section className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
      ...
      {issue?.compensationProposal?.submittedAt && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800 space-y-1">
          ...
          {issue.compensationProposal.directGearXpertReview ? (
            <p className="text-violet-800 font-medium">
              Đã nộp thẳng GearXpert — khách &amp; NCC được ghi nhận đồng ý chờ GX; admin duyệt &amp; quyết toán.
            </p>
          ) : null}
          {issue.compensationProposal.handledByAdmin?.fullName ? (
            <p className="text-slate-700">
              Admin phụ trách:{" "}
              <span className="font-semibold">{issue.compensationProposal.handledByAdmin.fullName}</span>
              {issue.compensationProposal.handledByAdmin.email
                ? ` (${issue.compensationProposal.handledByAdmin.email})`
                : ""}
            </p>
          ) : null}
