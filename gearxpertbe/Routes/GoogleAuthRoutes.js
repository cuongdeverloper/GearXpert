...
router.get('/auth/google', (req, res, next) => {
    ...
    const state = req.query.state || (isMobile ? 'mobile' : 'web');
...
router.get('/google/redirect', (req, res, next) => {
    passport.authenticate('google', (err, user, info) => {
        let isMobile = false;
        let mobileRedirectBase = 'gearxpertmobile://oauth2/callback';
        
        try {
            const stateObj = JSON.parse(req.query.state);
            if (stateObj && stateObj.mobile) {
                isMobile = true;
                if (stateObj.redirectUrl) mobileRedirectBase = stateObj.redirectUrl;
            }
        } catch (e) {
        ...
        if (err) {
            ...
            if (isMobile) {
                const separator = mobileRedirectBase.includes('?') ? '&' : '?';
                return sendMobileRedirect(`${mobileRedirectBase}${separator}error=${errorMessage}`);
            }
        ...
        if (!user) {
            if (isMobile) {
                const separator = mobileRedirectBase.includes('?') ? '&' : '?';
                return sendMobileRedirect(`${mobileRedirectBase}${separator}error=user_not_found`);
            }
        ...
        req.logIn(user, (loginErr) => {
            ...
            if (isMobile) {
                const separator = mobileRedirectBase.includes('?') ? '&' : '?';
                const redirectUrl = `${mobileRedirectBase}${separator}accessToken=${encodeURIComponent(accessToken)}`;
