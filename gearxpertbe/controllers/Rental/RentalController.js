...
exports.getRentalById = async (req, res) => {
  ...
  try {
    ...
    const devicesInfo = (itemsWithReports || []).map(ri => {
      const deviceObj = (ri.deviceId && typeof ri.deviceId === "object") ? ri.deviceId : {};
      const snapshotObj = ri.deviceSnapshot || {};
      return {
        name: deviceObj.name || snapshotObj.name || "Thiết bị",
        image: (deviceObj.images && deviceObj.images[0]) || (snapshotObj.images && snapshotObj.images[0]) || "",
  ...
  try {
    ...
    let rentals = await Rental.find({ customerId })
      ...
      .populate({
        ...
        select: "requestedEndDate requestedDays proposedExtraAmount status note createdAt rentalId",
    ...
    rentals = rentals.map(r => {
      ...
      obj.extensionRequests = (r.extensionRequests || []).map(er => er.toObject ? er.toObject() : er);
      return obj;
    });
...
exports.confirmReceived = async (req, res) => {
  ...
  try {
    ...
    // Kiểm tra đã xác nhận trước đó chưa
